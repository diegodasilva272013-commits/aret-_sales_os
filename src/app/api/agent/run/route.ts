...
export async function POST() {
  ...
  if (!scope) return NextResponse.json({ error: "Sin acceso" }, { status: 401 })
  if (scope.role !== "owner") return NextResponse.json({ error: "Solo el owner puede ejecutar el agente" }, { status: 403 })
