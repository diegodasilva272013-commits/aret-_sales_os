    ...
    <div className="min-h-screen p-8" style={{ background: "var(--background)" }}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: "Setters activos", value: team.length, icon: "👥" },
          ...
          <table className="w-full">
            <thead>
              <tr style={{ background: "var(--surface-2)" }}>
                {["Setter", "Email", "Prospectos", "Empresas", "Rol", "Ingresó", ""].map(h => (
                ...
                <tr key={setter.id} style={{ borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}
                  ...
                  <td className="px-5 py-4">
                    <span className="px-2 py-1 rounded-full text-xs"
                      style={{ background: setter.is_owner ? "rgba(108,99,255,0.15)" : "var(--surface-3)", color: setter.is_owner ? "var(--accent-light)" : "var(--text-muted)" }}>
                      {setter.is_owner ? "Admin" : "Setter"}
                    </span>
                  </td>
